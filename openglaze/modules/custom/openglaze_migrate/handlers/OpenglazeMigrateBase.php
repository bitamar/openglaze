...
/**
 * @file
 * Contains \OpenglazeMigrateBase.
 */

abstract class OpenglazeMigrateBase extends Migration {
  ...
  public function __construct() {
    ...
    if (!in_array($this->entityType, array('node', 'taxonomy_term'))) {
      throw new Exception('\OpenglazeMigration supports only nodes and terms.');
    }
  ...
  protected function getMigrateDirectory() {
    return variable_get('openglaze_migrate_directory', FALSE) ? variable_get('openglaze_migrate_directory') : drupal_get_path('module', 'openglaze_migrate');
  }
}
