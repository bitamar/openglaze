...
/**
 * @ngdoc directive
 * @name clientApp.directive:glazePie
 * @description
 * Display glaze materials as a d3 pie chart.
 */
angular.module('clientApp')
  .directive('glazePie', function () {
    return {
      retrict: 'EA',
      template: '<div class="pie"></div>',
      controller: function($scope) {
        var svg = d3.select(".pie").append("svg").append("g");

        svg.append("g").attr("class", "slices");
        svg.append("g").attr("class", "labels");
        svg.append("g").attr("class", "lines");

        var width = 960, height = 150, radius = Math.min(width, height) / 2;
        ...
        var pieData = [];
        var labels = [];
        for (var i in $scope.glaze.ingredients) {
          pieData.push({ label: $scope.glaze.ingredients[i].material.label, value: $scope.glaze.ingredients[i].amount / 100 });
          labels.push($scope.glaze.ingredients[i].material.label);
        }

        pieData.sort(function(a, b) { return a.amount > b.amount; });
        ...
        function change(data) {
          var duration = 0;
          var data0 = svg.select(".slices").selectAll("path.slice").data().map(function(d) { return d.data });
